...
import { editHabit } from "../../../utils/allHabitsUtils/editHabits";
    ...
    <div className="flex flex-col gap-2 mt-10 px-3">
      ...
      <div className="flex gap-4 justify-between items-center">
        ...
        <FontAwesomeIcon
          ...
          className="bg-mainColor mt-[1px] p-4 rounded-md text-white cursor-pointer bg-green-900 "
          icon={iconSelected}
          height={16}
          width={20}
        />
...
function Reminder({
  ...
  function ToggleSwitch() {
    return (
      <div
        className={`${isOn ? "bg-green-900" : "bg-slate-400"
          } w-16 h-[30px] relative rounded-lg flex`}
      >
        <div onClick={updateToggle} className="w-1/2 h-full  "></div>
        <div onClick={updateToggle} className="w-1/2 h-full "></div>
        <div
          className={`bg-white h-6  w-6 rounded-full ${isOn ? "right-[3px]" : "left-[3px]"
            } top-[3px] absolute`}
        ></div>
      </div>
    );
  ...
  return (
    <div className="w-full flex justify-center mt-9">
      <button
        ...
        className="bg-green-900 p-4 w-[98%] rounded-md text-white"
      >
        {buttonText}
      </button>
    </div>
